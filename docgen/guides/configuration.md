...
| `OnPlayerInit` | `((player: Player, rawData) -> ())?` | nil (no callback runs) | A callback invoked once per player right after their profile finishes loading, receiving the Player and their raw data table. Use it for per-player setup that needs the freshly loaded data, such as building leaderstats or one-time grants; any error it throws is caught and logged rather than blocking the load. |
